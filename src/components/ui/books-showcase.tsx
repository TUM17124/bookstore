    ...
    >
      ...
      {showDetailPanel && (
        <div
          ref={dpRef}
          aria-live="polite"
          className={`absolute right-[5%] top-6 bottom-4 z-[15] flex w-[min(520px,40%)] flex-col justify-start pointer-events-none
            @max-[760px]:right-auto @max-[760px]:left-1/2 @max-[760px]:top-auto @max-[760px]:bottom-3
            @max-[760px]:w-[min(560px,92cqw)] @max-[760px]:max-h-[38%] @max-[760px]:-translate-x-1/2
            ${panelVisible ? 'visible' : 'invisible delay-[500ms]'}`}
        >
          <h1
            className={`m-0 mb-2 line-clamp-2 max-w-full break-words text-[var(--bs-pink)] text-[clamp(22px,3.2cqw,42px)] font-extrabold leading-[1.12] tracking-[-0.02em] [overflow-wrap:anywhere] @max-[760px]:text-[20px] ${dpChild(50)}`}
          >
            {selectedCfg?.title}
          </h1>
